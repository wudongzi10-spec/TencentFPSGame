...
void AShooterPickup::RespawnPickup()
{
	...
	// re-enable collision and ticking directly so the pickup is functional again even if the
	// Blueprint OnRespawn event doesn't call FinishRespawn itself
	FinishRespawn();

	// call the BP handler for any extra respawn visuals
	BP_OnRespawn();
}
