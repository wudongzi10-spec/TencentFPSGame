...
void AShooterProjectile::NotifyHit(class UPrimitiveComponent* MyComp, AActor* Other, class UPrimitiveComponent* OtherComp, bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	// hide the projectile immediately so a stopped bullet doesn't appear frozen in mid-air,
	// while keeping the actor alive briefly for any deferred hit effects
...
void AShooterProjectile::ProcessHit(AActor* HitActor, UPrimitiveComponent* HitComp, const FVector& HitLocation, const FVector& HitDirection)
{
	...
	if (ACharacter* HitCharacter = Cast<ACharacter>(HitActor))
	{
		...
		{
			// friendly-fire filter: never damage another character on the shooter's own team.
			// The owner is still governed by bDamageOwner above, so self-damage projectiles keep working.
			bool bSameTeam = false;
			if (HitCharacter != GetOwner())
			{
				const ATencentFPSDemoCharacter* ShooterChar = Cast<ATencentFPSDemoCharacter>(GetInstigator());
				const ATencentFPSDemoCharacter* TargetChar = Cast<ATencentFPSDemoCharacter>(HitCharacter);
				bSameTeam = ShooterChar && TargetChar && ShooterChar->GetTeamByte() == TargetChar->GetTeamByte();
			}

			if (!bSameTeam)
			{
				// apply point damage so the target can read the impact location (used for headshots)
				AController* InstigatorController = GetInstigator() ? GetInstigator()->GetController() : nullptr;

				FHitResult PointHit;
				PointHit.ImpactPoint = HitLocation;
				PointHit.Location = HitLocation;
				PointHit.ImpactNormal = HitDirection;
				PointHit.Component = HitComp;
