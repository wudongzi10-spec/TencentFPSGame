 ...
 */
UCLASS(abstract)
class TENCENTFPSDEMO_API AShooterAIController : public AAIController
{
	GENERATED_BODY()
...
protected:
	...
	/** How often the NPC advances toward the nearest player when it has no sensed target */
	UPROPERTY(EditAnywhere, Category="Shooter|Seek", meta=(ClampMin=0.2))
	float SeekInterval = 0.8f;

	/** How close the NPC tries to get to a player while seeking without line of sight */
	UPROPERTY(EditAnywhere, Category="Shooter|Seek", meta=(ClampMin=50))
	float SeekAcceptanceRadius = 250.0f;

	/** Timer that drives the seek-nearest-player behavior */
	FTimerHandle SeekTimer;
...
protected:
	...
	/** Advances toward the nearest living player when no enemy is currently sensed */
