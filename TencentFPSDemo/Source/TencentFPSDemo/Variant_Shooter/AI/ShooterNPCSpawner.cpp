...
void AShooterNPCSpawner::BeginPlay()
{
	...
	{
		ShooterGameMode->RegisterSpawner(this);
	}
	
	// ensure we don't spawn NPCs if our initial spawn count is zero
	if (bAutoSpawn && SpawnCount > 0)
	{
...
AShooterNPC* AShooterNPCSpawner::SpawnNPCForWave()
{
	if (!HasAuthority())
	{
		return nullptr;
	...
	return SpawnNPCInternal();
}

AShooterNPC* AShooterNPCSpawner::SpawnNPCInternal()
{
